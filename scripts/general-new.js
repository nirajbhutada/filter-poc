...
var jsFolder = "/applications/homepage/scripts/";
var cssFolder = "/applications/homepage/css/";
...
setTimeout (function(){
			...
			if($(window).width() > 1500){
				$(window).scroll(function(){
					if($(window).scrollTop() > 495){
						$('.loanTopNav').css({ 'height': 157, 'position': 'fixed', 'left': 0, 'right': 0, 'top': 0, 'z-index': 999, 'transition': '350ms ease' });
	...
	*/
	...
	$('#interest-rate').parent().prepend('<span class="error alignRight">Min. 8</span>');
	...
	$("#interest-rate").keyup(function(e) {
		...
		if(interest>22) {
			alert('Please enter interest rate between 8 to 22');
			this.value=""
			return;
		}
		/*if(isNaN(interest)){
...
//Key Milestones
   ...
   $(this).next().slideUp();
	...
	$('#strUpNav li a, .strUpHomeBanner .strUpBtnGroup a').click(function() {
	...
	$('#strUpNav li a').on('click', function(e) {
		e.preventDefault();
		setTimeout(function(){ 
...
function yearArrayAmount(){
	...
	var interest_rateVal=0,interestTotalBal=0,principalBal=principal;
...
function Interest(){
	...
	var principalBal=principal,interest_rateVal=0,installment=0,interestTotalBal;
...
function slider(){
	var slider_range_minValue=10000,slider_range_min2Value=8,slider_range_min3Value=1;
